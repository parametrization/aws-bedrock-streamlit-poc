...
def processPDF(tab, file):
    uploadFileToS3(const.input_bucket, file.name, file.getvalue())
    
    # call textract
    session = boto3.Session()
    textract = session.client('textract', region_name=const.region_name)
    rsp = textract.detect_document_text(
            Document={
                'S3Object': {
                    'Bucket': const.input_bucket,
                    'Name': file.name,
                }
            })
    ...
    uploadFileToS3(const.output_bucket, const.output_dump + file.name + ".txt", bytes(dump, 'utf-8'))
    c2 = tab.container()
...
def uploader(tab):
    ...
    tab.divider()
    c1 = tab.container()
    df = pd.DataFrame({"Files":listFiles(const.input_bucket)})
